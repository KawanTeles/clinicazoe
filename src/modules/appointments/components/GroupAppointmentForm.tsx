...
export function GroupAppointmentForm({ insurances, professionals }: GroupAppointmentFormProps) {
  ...
  async function handleSubmit() {
    ...
    const [first, ...rest] = resolvedParticipants;
    const result = await previewRecurringAppointments({
      ...
      particularProduct: first.particularProduct,
  ...
  async function handleConfirmRecurring(skipDates: string[], overrides: Record<string, string>) {
    const representativePatient = participants[0]?.patient;
    if (!principalId || !time || !representativePatient) return;

    const resolvedParticipants: GroupParticipantInput[] = participants.map((p) => ({
      patientId: p.patient!.id,
      insuranceId: p.insuranceId,
    ...
    setSuccess({ count: result.createdCount ?? 0, skippedDates: result.skippedDates });
  }
    ...
    return (
      <div className="mx-auto flex max-w-xl flex-col items-center gap-4 rounded-2xl border border-primary/40 bg-card p-8 text-center shadow-card animate-fade-up">
        ...
        <h2 className="text-xl font-bold text-text-primary font-heading">
          {isRecurring ? `${success.count} sessão(ões) criadas na recorrência` : `${success.count} atendimentos criados nesta sessão`}
        </h2>
        {success.skippedDates && success.skippedDates.length > 0 && (
          <div className="w-full rounded-lg border border-warning/40 bg-warning/10 px-3 py-2 text-left text-xs text-text-secondary">
            <p className="mb-1 font-semibold text-text-primary">
              {success.skippedDates.length} data(s) não puderam ser criadas:
            </p>
            <ul className="flex flex-col gap-0.5">
              {success.skippedDates.map((s) => (
                <li key={s.date}>
                  {dateFormatter.format(new Date(`${s.date}T00:00:00`))} — {s.reason}
                </li>
              ))}
            </ul>
          </div>
  ...
  if (conflictOccurrences && principalId && representativeInsurance) {
    return (
      <div className="mx-auto flex max-w-4xl flex-col gap-3.5">
        ...
        <ConflictsReview
          ...
          modality={(representative?.modality || undefined) as Modality | undefined}
        />
      </div>
    );
  }
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-3.5">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-border/80 bg-card p-3.5 shadow-xs">
        ...
        <label className="flex items-center gap-2 text-xs font-semibold text-text-primary cursor-pointer">
          ...
          />
          Sessão Recorrente (Semanal, Quinzenal ou Mensal)
        </label>
        {isRecurring && (
          <div className="pt-1">
            <RecurrenceFields value={recurrence} onChange={setRecurrence} />
            <p className="mt-2 text-[11px] text-text-muted">
              Todos os participantes desta sessão (pacientes e profissionais) serão repetidos juntos, no mesmo padrão.
            </p>
          </div>
        )}
      </div>
      ...
      <Button className="h-10 w-full text-xs font-bold" isLoading={saving} disabled={!principalId || !date || !time} onClick={handleSubmit}>
        {isRecurring ? "Gerar prévia da recorrência" : "Criar atendimento conjugado"}
      </Button>
    </div>
  );
}
