"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { updateClinicSocialMedia } from "@/modules/settings/services/settings-actions";
import type { ClinicSettingsFormState } from "@/modules/settings/utils/form-state";

interface SocialMediaCardProps {
  data: ClinicSettingsFormState;
  onChange: (patch: Partial<ClinicSettingsFormState>) => void;
  readOnly: boolean;
}

export function SocialMediaCard({ data, onChange, readOnly }: SocialMediaCardProps) {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: "success" | "error"; text: string } | null>(null);

  async function handleSave() {
    setSaving(true);
    setMessage(null);
    const result = await updateClinicSocialMedia({
      instagram_url: data.instagram_url,
      facebook_url: data.facebook_url,
      linkedin_url: data.linkedin_url,
      youtube_url: data.youtube_url,
    });
    setSaving(false);
    if (result.error) {
      setMessage({ type: "error", text: result.error });
      return;
    }
    setMessage({ type: "success", text: "Redes sociais salvas." });
    router.refresh();
  }

  return (
    <Card className="h-full flex flex-col justify-between">
      <CardHeader className="py-3.5 px-5">
        <CardTitle className="text-xs font-bold uppercase tracking-wider text-[var(--primary)]">
          🌐 Redes Sociais
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4 sm:p-5 flex flex-col gap-4 flex-1 justify-between">
        <div className="flex flex-col gap-3.5">
          <p className="text-xs text-text-secondary leading-relaxed">
            Só aparecem no rodapé e na página de contato as redes preenchidas. Deixe em branco para ocultar.
          </p>
          <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            <Input
              label="Instagram"
              placeholder="https://instagram.com/suaclinica"
              value={data.instagram_url}
              disabled={readOnly}
              onChange={(e) => onChange({ instagram_url: e.target.value })}
            />
            <Input
              label="Facebook"
              placeholder="https://facebook.com/suaclinica"
              value={data.facebook_url}
              disabled={readOnly}
              onChange={(e) => onChange({ facebook_url: e.target.value })}
            />
            <Input
              label="LinkedIn"
              placeholder="https://linkedin.com/company/suaclinica"
              value={data.linkedin_url}
              disabled={readOnly}
              onChange={(e) => onChange({ linkedin_url: e.target.value })}
            />
            <Input
              label="YouTube"
              placeholder="Opcional"
              value={data.youtube_url}
              disabled={readOnly}
              onChange={(e) => onChange({ youtube_url: e.target.value })}
            />
          </div>
        </div>

        {message && (
          <p className={message.type === "success" ? "text-xs font-semibold text-success animate-fade-in" : "text-xs font-semibold text-danger animate-fade-in"}>
            {message.text}
          </p>
        )}

        {!readOnly && (
          <div className="flex justify-end pt-2 border-t border-border/40">
            <Button size="sm" isLoading={saving} onClick={handleSave} className="px-5 font-bold">
              Salvar Redes Sociais
            </Button>
          </div>
        )}
      </CardContent>
    </Card>
  );
}

