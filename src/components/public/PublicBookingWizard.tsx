...
/** Sem sessão, não dá pra oferecer a Lista de Espera (createWaitlistEntry
 * exige paciente autenticado) — aqui o visitante anônimo é direcionado
 * direto pro WhatsApp da clínica em vez de um botão que falharia. */
function NoAvailabilityState({ whatsappNumber }: { whatsappNumber?: string | null }) {
  const link = buildWhatsAppLink(
    whatsappNumber,
    "Olá! Gostaria de saber sobre horários disponíveis para atendimento.",
  );
  return (
    <div className="flex flex-col items-center gap-4 py-8 text-center">
      ...
      <div className="max-w-sm space-y-1.5">
        ...
        <p className="text-xs text-text-secondary leading-relaxed">
          Fale direto com a clínica pelo WhatsApp para verificar outras opções de horário.
        </p>
      </div>
      {link && (
        <a href={link} target="_blank" rel="noopener noreferrer">
          <Button className="font-bold">Falar no WhatsApp</Button>
        </a>
      )}
    </div>
  );
...
export function PublicBookingWizard({ specialties, initialProfessionals, whatsappNumber }: PublicBookingWizardProps) {
  ...
  // Form patient (sempre começa vazio — visitante anônimo)
  ...
  // Criação de conta opcional pós-agendamento
  ...
  async function loadInsurancesForSpecialty(specId: string) {
    ...
    setErrorMsg(null);
    try {
      const insList = await getBookableInsurances(specId, true);
  ...
  async function handleSelectInsurance(insId: string, insName: string) {
    ...
    try {
      ...
      const profs = await getBookableProfessionals(specId, insId, true);
  ...
  async function handleSelectProfessional(profId: string) {
    ...
    try {
      const [dates, pricingResult] = await Promise.all([
        getAvailableDates(profId, 45, true),
        getProfessionalPricing(profId, selectedInsuranceId, true),
      ]);
      ...
      if (requiresModality) {
        ...
        const duration = await getEffectiveDuration(profId, selectedInsuranceId, undefined, true);
  ...
  async function handleSelectModality(m: Modality) {
    ...
    setErrorMsg(null);
    try {
      const duration = await getEffectiveDuration(selectedProfessionalId, selectedInsuranceId, m, true);
      setEffectiveDuration(duration);
      setStep(STEP.date);
    } finally {
      setLoading(false);
  ...
  async function handleSelectDate(date: string) {
    ...
    setErrorMsg(null);
    try {
      const times = await getAvailableTimes(selectedProfessionalId, selectedInsuranceId, date, modality ?? undefined, true);
  ...
  async function handleFinalSubmit() {
    ...
    if (res.error) {
      ...
      setClaimEmail(patientEmail);
      setSuccessResult({ whatsappLink: res.whatsappLink, patientId: res.patientId, appointmentId: res.appointmentId });
    }
  }

  async function handleClaimAccount(e: React.FormEvent) {
    e.preventDefault();
    if (!successResult?.patientId || !successResult?.appointmentId) return;
    if (claimPassword.length < 6) {
      setClaimError("A senha precisa ter pelo menos 6 caracteres.");
      return;
    }
    setClaiming(true);
    setClaimError(null);

    const res = await claimPublicPatientAccount({
      patientId: successResult.patientId,
      appointmentId: successResult.appointmentId,
      email: claimEmail,
    ...
    const { error: signInError } = await signInWithPassword(claimEmail, claimPassword);
    setClaiming(false);
    ...
    return (
      <Card className="border-primary/50 bg-card shadow-card animate-fade-up">
        <CardContent className="p-8 text-center sm:p-12 space-y-6">
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-4">
            {successResult.whatsappLink && (
              <a
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl bg-[#0F7A3D] px-6 py-4 text-sm font-bold text-white transition-all hover:bg-[#0C6432] shadow-md"
              >
          ...
          {claimed ? (
            <div className="mx-auto max-w-md rounded-2xl border border-primary/40 bg-[var(--badge-bg)] p-5 text-left">
              <p className="text-sm font-semibold text-text-primary">Acesso criado com sucesso!</p>
              <p className="text-xs text-text-secondary mt-1">
                Sua senha foi definida. Entre na Área do Paciente para acompanhar seus atendimentos.
              </p>
              <a href="/cliente/login" className="inline-block mt-3">
                <Button size="sm" className="font-bold">Ir para o login</Button>
              </a>
            </div>
          ) : wantsAccount === null ? (
            <div className="mx-auto max-w-md rounded-2xl border border-border bg-card-elevated/70 p-6 text-left space-y-4">
              <div>
                <p className="text-sm font-bold text-text-primary font-heading">
                  Quer criar uma senha para acompanhar seus agendamentos?
                </p>
                <p className="text-xs text-text-secondary mt-1">
                  Com uma conta você recebe lembretes, acompanha o status do atendimento e pode reagendar por aqui.
                </p>
              </div>
              <div className="flex flex-col sm:flex-row gap-3">
                <Button className="w-full sm:w-auto font-bold" onClick={() => setWantsAccount(true)}>
                  Quero criar acesso
                </Button>
                <Button variant="secondary" className="w-full sm:w-auto font-bold" onClick={() => setWantsAccount(false)}>
                  Agora não
                </Button>
              </div>
            </div>
          ) : wantsAccount ? (
            <form onSubmit={handleClaimAccount} className="mx-auto max-w-md rounded-2xl border border-border bg-card-elevated/70 p-6 text-left space-y-4">
              <p className="text-sm font-bold text-text-primary font-heading">Criar acesso à Área do Paciente</p>
              {claimError && (
              ...
              />
              <div className="flex flex-col sm:flex-row gap-3">
                <Button type="submit" className="w-full sm:w-auto font-bold" isLoading={claiming}>
                  Criar acesso
                </Button>
                <Button
                  type="button"
        ...
        <Card className="animate-fade-up">
          <CardContent className="p-6">
            ...
            {loading ? (
              ...
              <NoAvailabilityState whatsappNumber={whatsappNumber} />
            ) : (
